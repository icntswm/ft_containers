    ...
    class Vector {
        ...
        public:
			...
			void insert(iterator pos, InputIt first, InputIt last, typename ft::enable_if<!std::numeric_limits<InputIt>::is_specialized>::type * = 0)
			{
				...
				else if (_size + raz > _capacity * 2)
				{
					temp = alloc.allocate(_size + raz);
					reserve(_size + raz);
				}
				try {
				...
				if (_size > 0) {
					for (size_type i = 0; i < _size; i++, j++) {
						if (j == counter) {
							for (InputIt i = first; i != last; i++, j++, first++) {
								alloc.construct(ptr + j, value);
							}
							--i;
						}
				...
				// 	for (size_type i = 0; i < _size; i++, j++) {
				// 		if (j == counter) {
				// 			for (size_type k = 0; k < raz; k++, j++, first++) {
				// 				alloc.construct(ptr + j, *first);
				// 			}
				// 			--i;
				// 			--j;
				// 		}
